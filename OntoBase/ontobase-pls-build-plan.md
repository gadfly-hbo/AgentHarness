...
1. 补 `pls-ontology-inventory.md`，把现有表/view 逐个映射为 ObjectType 和 PropertyType。
2. 补 `pls-matching-semantic-entrypoints.md`，固定 PLS 匹配读取入口。
3. 评估 OntoBase 是否需要独立 schema / 独立 SQLite / 独立服务；不默认落入 `DataBase/agentharness.sqlite`。
4. 当真实画像指标开始有数据后，优先更新真实画像指标相关 MetricDefinition。
5. 下一次 DataBase 新增 PLS 表/view/字段时，只在影响 PLS 首期协作绑定或语义解释时更新 OntoBase 文档。
