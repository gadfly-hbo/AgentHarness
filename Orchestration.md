...
本文定义 CDI（Controller-Domain Isolation，总控域隔离工程法）在 AgentHarness 中的执行方式。Codex 是总控；OpenCode、Kilo Code、Mimo Code、Kimi Code、Antigravity CLI 是固定域 worker。
...
| `console` | `antigravity` | `Console/**` | `docs/notes-console.md` |
...
Console 是控制平面和独立开发域，固定 assignee 为 Antigravity CLI。Codex 持有其 contract、跨域审批和集成所有权。Console 任务必须：

- 只修改 brief 授权的 `Console/**` 路径。
