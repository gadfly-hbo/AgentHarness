...
| `v_workpls_dimension_evidence` | view | 0 | WorkPLS 正式 Dimension Evidence 读取面，按唯一 snapshot、metric、unit 和 PLS 维度输出可追溯 evidence。 |
...
| `v_pls_audience_profile_snapshots` | view | 3 | 读取 PLS 渠道人群画像快照 metadata，不依赖 tag semantics 行是否存在。 |
...
| 读取渠道人群画像快照 metadata | `v_pls_audience_profile_snapshots` |
...
| WorkPLS 正式读取来源画像快照维度证据 | `v_workpls_dimension_evidence` |
...
`v_workpls_dimension_evidence` 只输出能唯一绑定到
`v_pls_audience_profile_snapshots` 的真实指标维度证据；绑定不上或绑定多条时 fail closed。
...
| 字段 | 含义 |
| --- | --- |
| `workspace_id` | PLS 工作空间ID，WorkPLS 读取时必须显式过滤。 |
| `snapshot_id` | 来源画像快照ID，当前等于 `profile_id`。 |
| `profile_id` | 平台画像指标所属的来源画像ID。 |
| `canonical_object_key` | PLS 统一渠道对象键。 |
| `data_version` | 来自唯一绑定 `v_pls_audience_profile_snapshots` 的来源数据版本。 |
| `metric_name` | 指标名称，例如 `share`、`tgi`、`count`、`index`、`score`。 |
| `metric_aggregation` | 当前固定为 `sum`。 |
| `dimension_key` | PLS 标准维度代码。 |
| `dimension_label` | PLS 标准维度中文名。 |
| `value` | `dimension_metric_sum`，只可在同一 `metric_name + unit + metric_aggregation` 内比较。 |
| `unit` | 真实画像指标单位，不为空时才输出。 |
| `profile_time_window` | 真实画像指标统计时间窗口，也用于严格绑定 snapshot。 |
| `source_batch_id` | 真实指标和 snapshot 来源批次，用于审计、重跑和版本隔离。 |
| `source_quality_flags_json` | 唯一绑定 snapshot 的质量标记 JSON array，不派生 WorkPLS quality status。 |
| `source_evidence_refs_json` | 支撑该维度证据的 `platform_profile_tag_metric` 来源记录 JSON array。 |
| `metric_row_count` | 聚合到该维度的真实指标行数量。 |
| `tag_type_count` | 聚合到该维度的标签类型数量。 |
| `tag_value_count` | 聚合到该维度的标签值数量。 |
| `avg_mapping_confidence` | 标签值到 PLS 维度映射的平均置信度。 |
| `latest_metric_updated_at` | 参与聚合的真实指标最近更新时间。 |
| `latest_mapping_updated_at` | 参与聚合的语义映射最近更新时间。 |

该 view 是 WorkPLS 后续 `Harness Portrait Consumption Contract 0.3.0` 的上游候选读取面。
它只基于 `platform_profile_tag_metrics` 真实指标链路，不使用旧
`v_pls_channel_dimension_features.dimension_score=sum(tag_score)` 诊断分数。
当前真实指标链路为 0 行，因此只能声明 schema/read-surface validated；real data coverage not validated。
...
| `workspace_id` | PLS 工作空间ID。 |
| `profile_id` | PLS 来源人群画像快照ID。 |
| `canonical_object_key` | 该画像所属的 PLS 渠道对象键。 |
| `data_version` | 来源数据版本；与 `workspace_id`、`profile_id` 共同构成业务唯一键。 |
| `source_batch_id` | 产生该画像记录的导入批次。 |
| `generated_at` | 来源画像数据生成或导出的时间。 |
| `time_window` | 人群画像统计覆盖的闭合日期窗口。 |
| `sample_size` | 生成该画像所依据的样本量；缺失时保持 `NULL`，不要补 0。 |
| `confidence` | 来源画像整体置信度，范围 0 到 1。 |
| `quality_flags_json` | 来源画像质量标记 JSON 数组，不派生 WorkPLS quality status。 |

这张 view 是画像快照 metadata 的最小读取面。它只返回 active profile，且必须存在同
`workspace_id`、`canonical_object_key`、`data_version` 的 active 渠道对象。它不投影
`tags_json`、`unmapped_fields_json`、`raw_json` 或对象展示字段；需要标签证据时继续读取
`v_pls_audience_tag_semantics`。
...
- WorkPLS 正式 Dimension Evidence 应读取 `v_workpls_dimension_evidence`；不同
  `metric_name`、`unit` 或 `metric_aggregation` 不得混合比较。
